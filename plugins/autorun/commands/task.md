...
With no scope it uses the configured default duration. A count means logical
Stop attempts, a duration uses `s`, `m`, or `h`, and count plus duration expires
at whichever limit is reached first. The remaining text is an optional
multiword reason.
...
may end that pause early. `/ar:task resume`, `/ar:go`, and `/ar:proc` also
resume enforcement.
