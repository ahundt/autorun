...
export_timestamp: 2026-02-07T23:58:55.645447
export_destination: /Users/athundt/.claude/clautorun/notes/2026-02-07-2358-plan-full-merge-of-plan-export-into-clautorun-dry-consolidation.md
---
...
### Commit 4: DRY refactor — standalone functions are single source of truth

Eliminate duplication between class methods and module-level functions. Where both exist, the standalone function becomes the single implementation and the class method is removed. Class callers switch to calling standalone functions directly.

**Remove 3 class methods (replaced by standalone function calls):**

| Removed Class Method | Standalone Replacement | Callers Updated |
|----------------------|----------------------|-----------------|
| `PlanExport.content_hash(self, path)` (line 465) | `get_content_hash(path)` (line 219) | Lines 615, 665: `self.content_hash(x)` → `get_content_hash(x)` |
| `PlanExport._embed_metadata(self, src, dest)` (line 689) | `embed_plan_metadata(src, session_id, dest)` (line 833) | Line 662: `self._embed_metadata(p, d)` → `embed_plan_metadata(p, self.ctx.session_id, d)` |
| `PlanExport._log(self, msg)` (line 706) | `log_warning(msg)` (line 244) | Lines 555, 565, 683, 686: `self._log(x)` → `log_warning(x)` |

Note: `embed_plan_metadata()` is the superset — it writes an extra `export_destination` field in frontmatter. `log_warning()` loads config from disk each call (slightly slower but acceptable for debug logging); remove "WARNING:" prefix to match `_log()` behavior.

**Remove 4 dead module-level functions (no callers after plan-export deletion):**

| Function | Lines | Why Remove |
|----------|-------|------------|
| `detect_hook_type()` | 212-216 | Dead, never called by daemon or class |
| `get_config_path()` | 228-230 | Trivial `return CONFIG_PATH` — callers use constant directly |
| `load_config()` | 233-236 | Thin wrapper — callers use `PlanExportConfig.load()` |
| `is_enabled()` | 239-241 | Thin wrapper — callers use `PlanExportConfig.load().enabled` |

**Keep all other standalone functions** — organized by purpose:

| Function | Lines | Purpose |
|----------|-------|---------|
| **Pure utilities (now called by class):** | | |
| `get_content_hash(file_path)` | 219-225 | SHA256 hash — single implementation, class calls this |
| `log_warning(message)` | 244-252 | Debug log — single implementation, class calls this |
| `embed_plan_metadata(plan_path, session_id, dest)` | 833-856 | YAML frontmatter — single implementation, class calls this |
| **Data recovery utilities:** | | |
| `get_most_recent_plan()` | 255-263 | Find most recent plan file by mtime |
| `get_plan_from_transcript(path)` | 266-305 | Extract plan path from session transcript JSONL |
| `get_plan_from_metadata(plan_path)` | 308-324 | Read session_id from plan YAML frontmatter |
| `find_plan_by_session_id(session_id)` | 327-334 | Find plan matching a session_id |
| **State access utilities:** | | |
| `load_tracking()` | 337-340 | Read export tracking from session state |
| `save_tracking(tracking)` | 343-346 | Write export tracking to session state |
| `record_export(plan_path, dest_path)` | 349-361 | Record a successful export |
| **Convenience API (construct PlanExport internally):** | | |
| `export_plan(plan_path, project_dir, session_id)` | 720-756 | One-call export for scripts/REPL |
| `export_rejected_plan(plan_path, project_dir, session_id)` | 759-795 | One-call rejected export |
| `handle_session_start(hook_input)` | 798-830 | Handle SessionStart from non-daemon path |

**Total: 3 class methods removed, 4 dead functions removed (~50 lines net). 13 standalone functions + 4 data recovery utils kept.**

Update `__all__` to remove 4 deleted names (`detect_hook_type`, `get_config_path`, `load_config`, `is_enabled`). The remaining module-level code is:
- Constants: `GLOBAL_SESSION_ID`, `DEFAULT_CONFIG`, `CONFIG_PATH`, `PLANS_DIR`, `DEBUG_LOG_PATH`
- Pure utilities: `get_content_hash`, `log_warning`, `embed_plan_metadata`
- Data recovery: `get_most_recent_plan`, `get_plan_from_transcript`, `get_plan_from_metadata`, `find_plan_by_session_id`
- State access: `load_tracking`, `save_tracking`, `record_export`
- Convenience API: `export_plan`, `export_rejected_plan`, `handle_session_start`
...
| `test_edge_cases.py` | Move + rewrite: `import plan_export` → `import clautorun.plan_export`. Delete tests for 4 removed functions (`detect_hook_type`, `get_config_path`, `load_config`, `is_enabled`), keep all other tests. |
...
| `clautorun/src/clautorun/plan_export.py` | 959 | Remove 3 class methods + 4 dead functions, update callers to use standalone functions, add migration | 2,4 | ~-50 |
