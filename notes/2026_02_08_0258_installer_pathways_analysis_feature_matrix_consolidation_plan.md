...
| Check project files | ❌ No validation of project structure | ✅ Checks `pyproject.toml` and `uv.lock` exist — catches common "forgot to clone" or "wrong directory" errors | ✅ Port from legacy: cheap validation check that prevents confusing downstream errors |
