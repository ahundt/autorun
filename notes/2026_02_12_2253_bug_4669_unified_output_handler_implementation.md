...
**Previous Issues (NOW RESOLVED in 7 commits)**:

**client.py** (✅ FIXED):
1. ✅ **4 different output paths consolidated** - Now single `output_hook_response()` (client.py:54-136)
2. ✅ **Exit code logic implemented** - Lines 118-136 handle both exit 0 and 2
3. ✅ **DRY achieved** - Single print(json.dumps()) at line 115, not duplicated
4. ✅ **Workaround implemented** - sys.exit(2) at line 133 when needed

**hook_entry.py** (✅ FIXED):
5. ✅ **Accepts exit 2** - Line 237: `if result.returncode not in (0, 2)`
6. ✅ **Passes through exit code** - Line 252: `sys.exit(result.returncode)`
7. ✅ **Workaround functional** - Exit 2 passed through, stderr preserved

**Remaining Issue**:
- ❌ **SessionStart:resume NoneType error** - Blocks verification (see below)
...
**Actual Implementation** (client.py:73-136):
...
def output_hook_response(response: dict | str, source: str = "daemon") -> None:
    ...
    # Handle string fallback (JSON decode errors)
    if isinstance(response, str):
        print(response)
        sys.exit(0)
        return

    # Extract decision (DRY - works for both Claude and Gemini formats)
...
**✅ COMPLETED (7 commits on feature/gemini-cli-integration)**:
- Steps 1-7 from original plan (hook_entry.py, config.py, client.py, core.py, etc.)
- hookSpecificOutput added to all responses
- Exit code 2 pathway implemented
- Auto-detection implemented

**❌ BLOCKING ISSUE FOUND**: SessionStart:resume NoneType Error

**Status Document**: `notes/2026_02_12_2153_bug_4669_implementation_status_and_pending_issues.md`

---

## CRITICAL: Fix NoneType Error (BLOCKING ALL VERIFICATION)
...
**Problem**: `ctx.payload` doesn't exist in EventContext (core.py:340-469). The `__getattr__` magic method returns `None` for unknown attributes, then `.get()` fails on `None`.

**Valid EventContext attributes** (core.py:397-484):
- `ctx.session_id` ✅ (line 398)
- `ctx.event` ✅ (line 402)
- `ctx.prompt` ✅ (line 406)
- `ctx.tool_name` ✅ (line 410)
- `ctx.tool_input` ✅ (line 414)
- `ctx.tool_result` ✅ (line 418)
- `ctx.transcript` ✅ (line 472)
- `ctx.has_justification` ✅ (line 480)
- `ctx.file_exists` ✅ (line 484)
- `ctx.payload` ❌ (doesn't exist - causes NoneType error)
...
**Why**: `ctx.event` always exists and contains the event name. EventContext design uses individual properties (`ctx.session_id`, `ctx.event`, etc.) rather than aggregated `payload` dict.

### Pre-Execution Verification

**CRITICAL**: Verify no other `ctx.payload` references exist before making the fix:
```bash
grep -rn "ctx\.payload" plugins/clautorun/src/clautorun/
# Expected: Only ONE match at plan_export.py:938
```
...
# Expected: No "SessionStart:resume hook error"
...
## Verification Steps (After NoneType Fix)

### Step 1: Verify Bug #4669 Workaround End-to-End

**Test**:
```bash
echo "test" > /tmp/test-bug4669.txt
rm /tmp/test-bug4669.txt
ls /tmp/test-bug4669.txt  # Should still exist
```

**Expected**:
- ✅ Tool BLOCKED (rm doesn't execute)
- ✅ File exists after rm attempt
- ✅ Trash suggestion in AI context
- ✅ No "hook error" message

### Step 2: Verify Stop Hook Fix

**Manual Test Command**:
```bash
# Simulate Stop hook manually
...
# Verify response includes hookSpecificOutput
```

**Expected**:
- ✅ Valid JSON with hookSpecificOutput field
- ✅ `hookEventName: "Stop"` in hookSpecificOutput
- ✅ No "JSON validation failed" error

### Step 3: Re-enable Plan Recovery

**File**: `plugins/clautorun/src/clautorun/plan_export.py:913-945`
**Action**: Remove temporary disable, uncomment original code
**Test**: Plan export works on new session
...
### 1. Fix hook_entry.py to Pass Through Exit Codes (CRITICAL) ✅ DONE
...
**Lines**: 56-136 (new functions before run_client())

**BEFORE**: (nothing, adding new functions)
...
**Lines**: 176-181 (actual current location)
...
**Lines**: 186-196 (actual current location)
...
**Path 4: Exception fail-open**
**Lines**: 235-245 (actual current location)
...
**File**: `plugins/clautorun/src/clautorun/core.py`
**Method**: `EventContext.respond()` (lines 509-607)

**Actual Implementation** (not _format_response - that name was conceptual):
```python
